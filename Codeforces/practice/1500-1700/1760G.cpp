...
void dfs(int src, int parent, vector<vector<pii>> &adjList, set<int> &s, int x, int b)
{
    if (src != b)
        s.insert(x);
    for (auto i : adjList[src])
        if (i.first != parent)
        {
            dfs(i.first, src, adjList, s, x ^ i.second, b);
        }
}

void dfs1(int src, int parent, vector<vector<pii>> &adjList, set<int> &s, int x, bool &res, int b)
{
    if (s.count(x) && src != b)
    {
        res = true;
    ...
    for (auto i : adjList[src])
    {
        if (i.first == b && ((x ^ i.second) != 0))
            continue;
        if (i.first != parent)
            dfs1(i.first, src, adjList, s, x ^ i.second, res, b);
    }
}
...
int32_t main()
{
    ...
    for (int tt = 1; tt <= t; tt++)
    {
        ...
        set<int> s;
        dfs(b, -1, adjList, s, 0, b);
        bool res = false;
        dfs1(a, -1, adjList, s, 0, res, b);
        res = res || s.count(0) > 0;
        cout << (res ? "YES\n" : "NO\n");
    }
    //---------------------------
