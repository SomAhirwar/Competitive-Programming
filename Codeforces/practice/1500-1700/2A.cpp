...
int32_t main()
{
    ...
    for (int tt = 1; tt <= t; tt++)
    {
        ...
        cin >> n;
        map<string, int> mp;
        map<string, vector<pii>> idx;
        int resScore = 0, resIdx = -1;
        string res = "";
        for (int i = 0; i < n; i++)
        {
            string s;
        ...
        for (auto &i : idx)
        {
            ...
            int nn = i.second.size();
            // debug(i.first);
            // debug(i.second);
            for (int ii = nn - 2; ii >= 0; ii--)
            {
                i.second[ii].second = min(i.second[ii].second, i.second[ii + 1].second);
            }
            // debug(i.first);
            // debug(i.second);
        }
        ...
        for (auto i : mp)
        {
            if (i.second > resScore)
            {
                resScore = i.second;
                res = i.first;
                pii temp = {i.second, 0};
                int index = lower_bound(idx[i.first].begin(), idx[i.first].end(), temp)->second;
                // cout << i.first << " " << i.second << " " << index << " " << resIdx << endl;

                // int tttt = lower_bound(idx[i.first].begin(), idx[i.first].end(), temp) - idx[i.first].begin();
                // debug(tttt);

                resIdx = index;
            }
            else if (i.second == resScore)
            {
                pii temp = {i.second, 0};
                int index = lower_bound(idx[i.first].begin(), idx[i.first].end(), temp)->second;
                // cout << i.first << " " << i.second << " " << index << " " << resIdx << endl;
