...
int lastIdx(vector<vector<int>> &arr, int x, int l, int h, int col)
{
    int res = -1;
    while (l <= h)
    {
...
int32_t main()
{
    ...
    for (int tt = 1; tt <= t; tt++)
    {
        ...
        int m;
        cin >> m;
        vector<vector<int>> arr(n, vector<int>(m)), org(n, vector<int>(m));
        for (int i = 0; i < n; i++)
            inputArr(j, arr[i]);
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++)
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            // debug(org[i]);
            // debug(brr);
            int l = 0, h = n - 1, resTemp = 0;
            for (int j = 0; j < m; j++)
            {
                int firstI = firstIdx(arr, org[i][j], l, h, j);
                int lastI = lastIdx(arr, org[i][j], l, h, j);
                // cout << firstI << " - " << lastI << endl;
                if (firstI < l || firstI > h || lastI < l || lastI > h || firstI > lastI)
                    break;
                resTemp++;
                l = firstI, h = lastI;
            }
