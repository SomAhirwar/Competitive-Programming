...
int32_t main()
{
    ...
    for (int tt = 1; tt <= t; tt++)
    {
        ...
        srt(arr);
        vector<int> prof[n];
        // vector<int> profOf[MX];
        // for (int i = 0; i < n; i++)
        //     profOf[arr[i]].push_back(i);
        // debug(arr);

        // debug(prof);
        vector<int> cntArr(m + 1);
        int cnt = m;
        int j = 0;
        int res = LLONG_MAX;
        for (int i = 0; i < n; i++)
