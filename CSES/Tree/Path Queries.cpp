...
public:
    ...
    int query(int node, int left, int right, int l, int h)
    {
        ...
        int mid = getMid(left, right);
        return query(leftChild(node), left, mid, l, h) + query(rightChild(node), mid + 1, right, l, h);
    }
};
...
int queryHLD(int node1, int node2, segmentTree &s, int n)
{
    ...
    while (head[node1] != head[node2])
    {
        ...
        res += s.query(0, 0, n - 1, indexOf[head[node1]], indexOf[node1]);
        // cout << res << " " << indexOf[head[node1]] << " " << indexOf[node1] << " " << node1 << " " << node1 << " " << endl;
    ...
    res += s.query(0, 0, n - 1, indexOf[node2], indexOf[node1]);
    return res;
}
