...
int calcPath(vector<vector<int>> &adjList, int src, int parent, vector<int> &pathArr, vector<int> &lcaArr, vector<int> &res)
{
    int paths = pathArr[src];
    for (int i : adjList[src])
    {
        if (i == parent)
            continue;
 
        paths += calcPath(adjList, i, src, pathArr, lcaArr, res);
    }
 
    res[src] = paths - lcaArr[src];
    return paths - 2 * lcaArr[src];
}
...
int32_t main()
{
    ...
    vector<int> pathArr(n), lcaArr(n), res(n); //diffrance Array
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        lcaArr[commonAncestor]++;
        pathArr[u]++;
        pathArr[v]++;
    }
 
    calcPath(adjList, 0, -1, pathArr, lcaArr, res);
    for (int i : res)
        cout << i << " ";
